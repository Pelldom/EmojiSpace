"""
EmojiSpace CLI v1 — player-facing test harness.
Based on run_game_engine_cli.py. Preserves harness behavior; hides internal IDs and log spam;
uses Emoji Profiles for all named entities; menu-driven flows; file logging unchanged.
"""
...
from knowledge_state import (
    ...
    get_system_view,
    mark_system_scanned_local,
    SYSTEM_LEVEL_VISITED,
)
...
# ============================================================================
# PLAYER-FACING DISPLAY — format_entity_name (Emoji Profiles)
# ============================================================================

def _normalize_entity_for_display(entity: object) -> object:
    """Normalize dict or object for build_emoji_profile_parts (emoji_id, tier, tags, category)."""
    if entity is None:
...
def format_entity_name(entity: object, name: str) -> str:
    """
    Format named entity for display: [primary] NAME [tier] [secondary] [secondary] ...
    Primary, tier, secondaries optional. Preserve order; never sort; never truncate.
    Uses build_emoji_profile_parts. If no primary: NAME [tier] [secondary] ...
    """
    if not name:
        return ""
    try:
...
def _entity_display(entity: object, name: str, visible: bool = True) -> str:
    """Player-facing entity display; when not visible (fog of war) return name only."""
    if not visible:
        return name
...
def _destination_display_entity(destination: object) -> object:
    """Adapter for destination entities used with Emoji Profile builder.

    If the underlying destination already has an emoji_id, return it as-is so all
    authoritative fields (population, economies, situations) are preserved.

    If emoji_id is missing, wrap in a lightweight namespace that:
    - injects emoji_id based on destination_type (location_planet/location_station/...)
    - preserves destination_type, population, primary/secondary economies,
      destination situations, tier, and tags so the builder can compose the full
      profile.
    """
    if destination is None:
        return SimpleNamespace(emoji_id=None, tier=None, tags=[])
    eid_raw = getattr(destination, "emoji_id", None)
    eid = (eid_raw or "").strip() if isinstance(eid_raw, str) else ""
    if eid:
        return destination
    dtype_raw = getattr(destination, "destination_type", None)
    dtype = (dtype_raw or "").strip() if isinstance(dtype_raw, str) else ""
    if not dtype:
        return destination
    primary_econ = getattr(destination, "primary_economy_id", None) or getattr(destination, "primary_economy", None)
    secondary_econs = (
        getattr(destination, "secondary_economy_ids", None)
        or getattr(destination, "secondary_economies", None)
        or []
    )
    dest_situations = (
        getattr(destination, "active_destination_situations", None)
        or getattr(destination, "destination_situations", None)
        or []
    )
    return SimpleNamespace(
        ...
        primary_economy_id=primary_econ,
        secondary_economy_ids=list(secondary_econs) if isinstance(secondary_econs, (list, tuple)) else [],
        active_destination_situations=list(dest_situations)
        if isinstance(dest_situations, (list, tuple))
        else [],
...
def _format_name_with_profile(entity: object, name: str, visible: bool) -> str:
    """Alias for player-facing display; uses format_entity_name when visible."""
    return _entity_display(entity, name, visible)


def _format_good_name_with_profile(good_or_row: object, name: str, visible: bool = True) -> str:
    """Goods use same [primary] NAME [tier] [secondary]... via format_entity_name (category=primary, tags=secondary)."""
    if not visible or not name:
        return name or ""
    return format_entity_name(_normalize_entity_for_display(good_or_row), name)


def _resolve_good_entity_for_display(engine: GameEngine, sku_id: str) -> object | None:
    """
    Resolve a good-like entity (base or derived) suitable for Emoji Profile display.

    - Base goods: return the catalog Good.
    - Derived goods with modifier prefixes (luxury_, hazardous_, propaganda_, weaponized_, stolen_, cybernetic_):
      reconstruct from the base Good plus the modifier tag, preserving category and tags.
    """
    if not sku_id:
        return None
    try:
    ...
    # 1) Try exact SKU from catalog (base goods).
    try:
        return catalog.good_by_sku(sku_id)
    except KeyError:
        pass

    # 2) Derived SKU: strip possible_tag-style prefix and reconstruct from base good.
    possible_tags = {
        "luxury",
        "weaponized",
        "counterfeit",
    ...
    if "_" not in sku_id:
        return None
    prefix, remainder = sku_id.split("_", 1)
    if prefix not in possible_tags or not remainder:
        return None
    try:
        base_good = catalog.good_by_sku(remainder)
    except KeyError:
        return None

    # Build a lightweight entity carrying category + full tags (base + modifier) and a derived name.
    tags = list(base_good.tags)
    if prefix not in tags:
        tags.append(prefix)
    derived_name = f"{prefix.title()} {base_good.name}"
    from types import SimpleNamespace

    return SimpleNamespace(
        sku=sku_id,
        name=derived_name,
        category=base_good.category,
        tags=tags,
    )
...
def _get_good_display_name(engine: GameEngine, sku_id: str) -> str:
    ...
    if not sku_id:
        return "Unknown Good"
    try:
        entity = _resolve_good_entity_for_display(engine, sku_id)
        if entity is None:
            return sku_id
        name = getattr(entity, "name", None) or _get_good_name(engine, sku_id)
        return _format_good_name_with_profile(entity, name, True)
    except Exception:  # noqa: BLE001
        return sku_id
    ...
    if reward_summary:
        ...
        for reward in reward_summary:
            ...
            if field == "credits":
                ...
                try:
                    from hull_utils import get_hull_display_name as _hull_name
                except Exception:
                    _hull_name = lambda x: x or "Hull"
                lines.append(f"Hull voucher: {_hull_name(hull_id)}")
...
def _prompt_admin_override() -> dict | None:
    ...
    for idx, hull in enumerate(hulls_list, start=1):
        ...
        display = format_entity_name(hull_entity, name)
        print(f"  [{idx}] {display}")
...
def _show_player_info(engine: GameEngine) -> None:
    ...
    cargo_display = _format_cargo_for_display(engine, detail.get("cargo_manifest", {}))
    ...
    print(f"Cargo manifest: {cargo_display}")
    ...
    if ship_info:
        ...
        print(f"  {ship_headline}")
        hull_data = ship_info.get("hull_integrity")
        if isinstance(hull_data, dict):
            hull_int = hull_data.get("current")
            hull_max = hull_data.get("max")
        else:
            hull_int = ship_info.get("current_hull_integrity")
            hull_max = ship_info.get("max_hull_integrity")
        if hull_int is not None and hull_max is not None:
            print(f"  Hull: {hull_int}/{hull_max}")
        print(f"  Fuel: {ship_info.get('current_fuel', detail.get('fuel_current', 0))}/{ship_info.get('fuel_capacity', 0)}")
    ...
    if not warehouses:
        ...
        for index, row in enumerate(warehouses, start=1):
            ...
            goods_display = _format_cargo_for_display(engine, goods)
            print(
                f"  {index}) {dest_display} capacity={capacity} "
                f"used={used} available={available} cost/turn={cost} goods={goods_display}"
            )
...
def _show_ships_and_modules(engine: GameEngine) -> None:
    ...
    if active_ship:
        ...
        if cargo:
            print(f"  Cargo: {_format_cargo_for_display(engine, cargo)}")
        else:
            print("  Cargo: none")
    else:
        print("\nACTIVE SHIP: None")
    ...
    if inactive_ships:
        ...
        for ship in inactive_ships:
            display = _get_ship_headline(engine, ship)
            print(f"  {display}")
            dest_id = ship.get("destination_id")
            if dest_id:
                dest_name = _get_destination_name(engine, dest_id)
                dest_obj = _get_destination_object(engine, dest_id)
                dest_ent = _destination_display_entity(dest_obj) if dest_obj else SimpleNamespace(emoji_id=None, tier=None, tags=[])
                visible = dest_id in _visited_destination_ids(engine)
                at_display = format_entity_name(dest_ent, dest_name) if visible else dest_name
                print(f"    At: {at_display}")
...
def _change_active_ship(engine: GameEngine) -> None:
    ...
    for idx, ship in enumerate(inactive_ships, start=1):
        ...
        dest_id = ship.get("destination_id")
        if dest_id:
            at_name = _get_destination_name(engine, dest_id)
            dest_obj = _get_destination_object(engine, dest_id)
            dest_ent = _destination_display_entity(dest_obj) if dest_obj else SimpleNamespace(emoji_id=None, tier=None, tags=[])
            at_display = format_entity_name(dest_ent, at_name) if dest_id in _visited_destination_ids(engine) else at_name
        else:
            at_display = "Unknown"
        print(f"  {idx}) {display} at {at_display}")
...
def _transfer_cargo(engine: GameEngine) -> None:
    ...
    try:
        ...
        if result.get("ok"):
            good_name = _get_good_display_name(engine, sku_id)
            print(f"Transferred {qty} units of {good_name}.")
        else:
            error = result.get("error", "unknown_error")
...
def _abandon_mission(engine: GameEngine) -> None:
    ...
    print("\nActive Missions:")
    for idx, mission in enumerate(active_missions, start=1):
        title = mission.get("title", mission.get("mission_type", "Mission"))
...
def _dismiss_crew_menu(engine: GameEngine) -> None:
    ...
    if isinstance(detail, dict):
        ...
        if result_detail.get("ok"):
            ...
            npc_name = _get_npc_name(engine, npc_id)
            print(f"Crew member {npc_name} dismissed and relocated to:")
            sys_id = relocated.get("system_id")
            if sys_id:
                system_obj = engine.sector.get_system(sys_id)
                system_name = _get_system_name(engine, sys_id)
                sys_display = _format_name_with_profile(system_obj or SimpleNamespace(emoji_id=None, tier=None, tags=[]), system_name, True)
                print(f"  System: {sys_display}")
            dest_id = relocated.get("destination_id")
            loc_id = relocated.get("location_id")
            if dest_id:
                dest_name = _get_destination_name(engine, dest_id)
                dest_obj = _get_destination_object(engine, dest_id)
                dest_ent = _destination_display_entity(dest_obj) if dest_obj else SimpleNamespace(emoji_id=None, tier=None, tags=[])
...
def _show_system_info(engine: GameEngine) -> None:
    ...
    print("SYSTEM INFO")
    print(f"  System: {system_display}")
    if system_visited:
        gov_id = detail.get("government_id")
        gov = engine.government_registry.get_government(gov_id) if gov_id else None
        gov_label = gov.name if gov else (gov_id or "Unknown")
        print(f"  Government: {gov_label}")
    ...
    if system_visited:
        situations = detail.get("active_system_situations")
        situations_str = ", ".join(situations) if isinstance(situations, list) else str(situations or "none")
        print(f"  Active situations: {situations_str}")
        system = engine.sector.get_system(system_id)
        if system is None:
    ...
    flags = detail.get("active_system_flags")
    flags_str = ", ".join(flags) if isinstance(flags, list) else str(flags or "none")
    print(f"  Active flags: {flags_str}")
    print("  Reachable systems:")
    for row in detail.get("reachable_systems", []):
        if not row.get("in_range", False):
            continue
        ...
        if system_obj is not None:
            # Upgrade in-range systems to scanned_local based on authoritative reachability.
            mark_system_scanned_local(engine.player_state, system_obj, current_turn=get_current_turn())
        view = get_system_view(
            engine.player_state,
            engine.sector,
            system_id=system_id_reachable,
            current_system_id=engine.player_state.current_system_id,
            current_turn=get_current_turn(),
        )
        # Render from knowledge view only: [primary] NAME [government]; no population unless visited.
        entity = SimpleNamespace(
            emoji_id=view.primary_emoji_id,
            tier=view.population_display if view.level == SYSTEM_LEVEL_VISITED else None,
            tags=[],
        )
        base_name = view.name or system_name_reachable
        display = format_entity_name(entity, base_name)
        if view.government_display:
            gov = engine.government_registry.get_government(view.government_display)
            gov_label = gov.name if gov else view.government_display
            display = f"{display} {gov_label}"
        print(f"    {display} ({distance:.1f} LY)")
...
def _show_destination_info(engine: GameEngine) -> None:
    ...
    dest_entity = _destination_display_entity(destination_object) if destination_object else SimpleNamespace(emoji_id=None, tier=None, tags=[])
    dest_display = format_entity_name(dest_entity, destination_name) if destination_visited else destination_name
    print("DESTINATION INFO")
    print(f"  Destination: {dest_display}")
    system_id = detail.get("system_id") or engine.player_state.current_system_id
    if system_id:
        system_obj = engine.sector.get_system(system_id)
        system_name = _get_system_name(engine, system_id)
        sys_visible = system_id in _visited_system_ids(engine)
        system_display = _format_name_with_profile(
            system_obj or SimpleNamespace(emoji_id=None, tier=None, tags=[]),
            system_name,
            sys_visible,
        ...
        print(f"  System: {system_display}")
    ctx = {
        "primary_economy": detail.get("primary_economy"),
        "secondary_economies": detail.get("secondary_economies") or [],
    }
    has_market = bool(detail.get("market_attached") and destination_object and getattr(destination_object, "market", None) is not None)
    market_lines = _format_market_profile_lines(engine, ctx, has_market)
    for line in market_lines:
        print(f"  {line}")
    if destination_visited:
        dest_situations = detail.get("active_destination_situations")
        dest_sit_str = ", ".join(dest_situations) if isinstance(dest_situations, list) else str(dest_situations or "none")
        print(f"  Active destination situations: {dest_sit_str}")
    else:
    ...
    if not destination_visited:
        ...
        for row in locations:
            ...
            name = (location_type or "Unknown").replace("_", " ").title()
            location_entity = SimpleNamespace(
                emoji_id=f"location_{location_type}" if location_type else None,
                tier=None,
                tags=[],
            )
            display = format_entity_name(location_entity, name)
            print(f"    {display}")
...
def _print_encounter_resolution_block(
    ...
    if cargo_sku and cargo_qty:
        print()
        print("Cargo Rewards:")
        cargo_name = _get_good_display_name(engine, cargo_sku) if engine else cargo_sku
        print(f"  {cargo_name} x{cargo_qty}")
...
def _resolve_pending_encounter(engine: GameEngine) -> None:
    ...
    while engine.has_pending_encounter():
        ...
        print("\n--- ENCOUNTER ---")
        encounter_description = pending_info.get("encounter_description") or "Unknown"
        encounter_entity = pending_info.get("npc_ship_info") or SimpleNamespace(emoji_id=None, tier=None, tags=[])
        encounter_display = format_entity_name(encounter_entity, encounter_description)
        print(f"Encounter: {encounter_display}")
        npc_ship_info = pending_info.get("npc_ship_info")
        npc_display_name = None
        if npc_ship_info:
            hull_name = npc_ship_info.get("hull_name", "Unknown")
            npc_display_name = format_entity_name(npc_ship_info, hull_name)
            print(f"Ship: {npc_display_name}")
        desc_lower = (encounter_description or "").lower()
        if "patrol" in desc_lower or "customs" in desc_lower or "border" in desc_lower:
            print("They demand inspection.")
        elif "derelict" in desc_lower or "station" in desc_lower:
            print("No response from the contact.")
        else:
            print("The ship hails you.")
...
def _travel_menu(engine: GameEngine) -> None:
    ...
    while True:
        ...
        # Current destination: use knowledge-state destination view
        if isinstance(engine.player_state.current_destination_id, str):
            ...
            dest_obj = _get_destination_object(engine, engine.player_state.current_destination_id)
            dest_entity = _destination_display_entity(dest_obj) if dest_obj else SimpleNamespace(emoji_id=dest_view.primary_emoji_id, tier=None, tags=[])
            dest_name = dest_view.name or current_destination_name
            dest_display = format_entity_name(dest_entity, dest_name)
        else:
            dest_display = current_destination_name
        # Current system: use knowledge-state system view
        sys_view = get_system_view(
            engine.player_state,
            engine.sector,
            system_id=current_system.system_id,
            current_system_id=current_system.system_id,
            current_turn=get_current_turn(),
        )
        sys_entity = current_system
        sys_name = sys_view.name or _get_system_name(engine, current_system.system_id)
        system_display = format_entity_name(sys_entity, sys_name)
        print(f"Current system: {system_display}")
        print(f"Current destination: {dest_display}")
        print("\n0) Back")
        ...
        if mode == "0":
            ...
            for index, row in enumerate(reachable, start=1):
                ...
                # Mark in-range systems as scanned_local
                mark_system_scanned_local(engine.player_state, system, current_turn=get_current_turn())
                view = get_system_view(
                    engine.player_state,
                    engine.sector,
                    system_id=system_id,
                ...
                base_name = view.name or _get_system_name(engine, system_id)
                # Entity shape for emoji profile: primary + government emoji (no tier unless visited)
                star_type = (view.primary_emoji_id or "").replace("star_", "", 1).strip() or None
                entity = SimpleNamespace(
                    star_type=star_type,
                    government_id=view.government_display,
                    population=view.population_display if view.level == SYSTEM_LEVEL_VISITED else None,
                )
                display = format_entity_name(entity, base_name)
                distance = row.get("distance_ly", 0.0)
                print(f"{index}) {display} ({distance:.1f} LY)")
            ...
            if not result.get("hard_stop"):
                print(f"Arrived at {target_display}")
            ...
            while result.get("hard_stop") is True:
                ...
                if hard_stop_reason == "pending_encounter_decision":
                    ...
                    if pending_encounter:
                        ...
                        enc_entity = pending_encounter.get("npc_ship_info") or SimpleNamespace(emoji_id=None, tier=None, tags=[])
                        print(f"Encounter: {format_entity_name(enc_entity, enc_desc)}")
                        npc_info = pending_encounter.get("npc_ship_info")
                        if npc_info:
                            print(f"Ship: {format_entity_name(npc_info, npc_info.get('hull_name', 'Unknown'))}")
                        enc_lower = (enc_desc or "").lower()
                        if "patrol" in enc_lower or "customs" in enc_lower or "border" in enc_lower:
                            print("They demand inspection.")
                        else:
                            print("The ship hails you.")
            ...
            for index, destination in enumerate(destinations, start=1):
                ...
                # [type] NAME [population/tier] [economy] [economy...] via emoji profile; no situations for list.
                dest_ent = SimpleNamespace(
                    destination_type=view.destination_type or "planet",
                    population=view.population_display,
                    primary_economy_id=view.primary_economy_display,
                    secondary_economy_ids=view.secondary_economies_display or [],
                    active_destination_situations=[],
                )
                name = view.name or getattr(destination, "display_name", dest_id)
                display_name = format_entity_name(dest_ent, name)
            ...
            while result.get("hard_stop") is True:
                ...
                if hard_stop_reason == "pending_encounter_decision":
                    ...
                    if pending_encounter:
                        print("\n--- ENCOUNTER ---")
                        enc_desc2 = pending_encounter.get("encounter_description", "Unknown")
                        enc_entity2 = pending_encounter.get("npc_ship_info") or SimpleNamespace(emoji_id=None, tier=None, tags=[])
                        print(f"Encounter: {format_entity_name(enc_entity2, enc_desc2)}")
                        npc_info2 = pending_encounter.get("npc_ship_info")
                        if npc_info2:
                            print(f"Ship: {format_entity_name(npc_info2, npc_info2.get('hull_name', 'Unknown'))}")
                        enc_lower2 = (enc_desc2 or "").lower()
                        if "patrol" in enc_lower2 or "customs" in enc_lower2 or "border" in enc_lower2:
                            print("They demand inspection.")
                        else:
                            print("The ship hails you.")
                        options = pending_encounter.get("options", [])
...
def _location_entry_menu(engine: GameEngine) -> None:
    while True:
        ...
        for index, location in enumerate(locations, start=1):
            ...
            name = (loc_type or "Unknown").replace("_", " ").title()
            location_entity = SimpleNamespace(
...
def _format_current_location_display(engine: GameEngine) -> str:
    """Return current location name with emoji profile: [primary] Name. Use for all LOCATION/SHIPDOCK headers."""
    ...
    return format_entity_name(location_entity, name)
                                    ...
                                    if accept_result.get("ok"):
                                        ...
                                        if accept_detail and isinstance(accept_detail, dict):
                                            if accept_detail.get("accepted"):
                                                title = selected_mission.get("name") or selected_mission.get("title") or selected_mission.get("mission_type") or mission_id
                                                print(f"Mission accepted: {title}")
                                            else:
                    ...
                    if detail and detail.get("missions"):
                        ...
                        for mission in missions:
                            if isinstance(mission, dict):
                                ...
                                print(f"  {type_display} (Tier {mission.get('mission_tier')})")
            ...
            if isinstance(interaction_result.get("rumor_type"), str):
                ...
                if isinstance(hint, dict):
                    ...
                    if isinstance(system_id, str) and system_id:
                        sys_name = _get_system_name(engine, system_id)
                        if isinstance(destination_id, str) and destination_id:
                            dest_name = _get_destination_name(engine, destination_id)
                            print(f"Hint: {sys_name} — {dest_name}")
                        else:
                            print(f"Hint: {sys_name}")
                ...
                if confirm == "y":
                    ...
                    if accept_result.get("ok"):
                        ...
                        base_title = (
                            ...
                            or mission_dict.get("title")
                            or mission_dict.get("mission_type")
                        )
                        if accept_detail and isinstance(accept_detail, dict):
                            if accept_detail.get("accepted"):
                                # Prefer mission title/type; fall back to id only if needed.
                                title = base_title or accept_detail.get("mission_id") or mission_id
                                if title:
                                    print(f"Mission accepted: {title}")
                                else:
                            ...
                            title = base_title or mission_id
                            if title:
                                print(f"Mission accepted: {title}")
                            else:
                                print("Mission accepted.")
                ...
                m = interaction_result.get("missions") or []
                mission_obj = m[0] if isinstance(m, list) and len(m) and isinstance(m[0], dict) else None
                title = (mission_obj.get("name") or mission_obj.get("title") or mission_obj.get("mission_type") or interaction_result.get("mission_id", "Mission")) if mission_obj else interaction_result.get("mission_id", "Mission")
                print(f"Mission accepted: {title}")
            else:
...
def _warehouse_location_menu(engine: GameEngine) -> None:
    ...
    while True:
        ...
        _print_warehouse_profile(engine, profile)
        print(f"LOCATION: {_format_current_location_display(engine)}")
        print("1) Rent space")
        ...
        if raw_action == "3":
            profile = _build_warehouse_profile(engine)
            stored = profile.get("stored_goods", {}) or {}
            if not isinstance(stored, dict) or not stored:
                print("No goods in warehouse to withdraw.")
                continue
            sku_list = sorted(stored.keys())
            for index, sku_id in enumerate(sku_list, start=1):
                display_name = _get_good_display_name(engine, sku_id)
                print(f"{index}) {display_name} units={stored[sku_id]}")
            raw_index = input("Select good index: ").strip()
            try:
                selected_index = int(raw_index)
            except ValueError:
                print("Invalid index.")
                continue
            if selected_index < 1 or selected_index > len(sku_list):
                print("Invalid index.")
                continue
            selected_sku_id = sku_list[selected_index - 1]
            raw_qty = input("Quantity: ").strip()
            try:
                quantity = int(raw_qty)
            except ValueError:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="EmojiSpace CLI v1 - Player-facing playtest harness")
    ...
    # File logging: set path before engine so logs are ready
    ...
    # Run entire session with console log suppression (file logging unchanged)
    with _suppress_logger_console():
        ...
        _playtest_context = {"playtest_logger": playtest_logger, "verbose": args.verbose}
        print("\n" + "=" * 60)
        print("EMOJISPACE")
        print("=" * 60 + "\n")
        _render_galaxy_map(engine.sector, engine=engine)
...
def _render_galaxy_map(sector, width: int = 80, height: int = 30, engine: Optional[GameEngine] = None) -> None:
        ...
        if grid[row][col] is not None:
            ...
            if collision_entry is None:
                ...
                if existing_index > 0:
                    ...
                    collisions.append((row, col, [(existing_index, getattr(existing_sys, "system_id", ""), existing_sys.name)]))
                    collision_entry = len(collisions) - 1
                else:
                    collisions.append((row, col, []))
                    collision_entry = len(collisions) - 1
            collisions[collision_entry][2].append((index, getattr(system, "system_id", ""), system.name))
            grid[row][col] = "*"
        else:
            grid[row][col] = label
    ...
    # Print legend (player-facing: display names only, no raw IDs)
    print("\nLEGEND:")
    for index, system in enumerate(sorted_systems, start=1):
        ...
        display_name = _entity_display(system, system.name, visible)
        print(f"  {label} {display_name} (x={system.x:.3f}, y={system.y:.3f})")
...
def _print_current_system_destinations(engine: GameEngine) -> None:
    ...
    if system is None:
        return
    destinations = sorted(system.destinations, key=lambda d: getattr(d, "destination_id", ""))
    ...
    for index, destination in enumerate(destinations, start=1):
        ...
        # Same local_visible format as travel menu: [type] NAME [population/tier] [economy] [economy...]
        dest_ent = SimpleNamespace(
            destination_type=view.destination_type or "planet",
            population=view.population_display,
            primary_economy_id=view.primary_economy_display,
...
def _economy_display(economy_id: str) -> str:
    """Player-facing economy label (no raw ids)."""
    if not economy_id:
        return ""
    return str(economy_id).replace("_", " ").title()


def _format_market_profile_lines(engine: GameEngine, ctx: Dict[str, Any], has_market: bool) -> List[str]:
    """Build player-facing market profile lines for destination summary: economy, produces, consumes. No raw IDs."""
    lines: List[str] = []
    primary = ctx.get("primary_economy")
    secondaries = ctx.get("secondary_economies") or []
    if primary:
        economy_str = _economy_display(primary)
        if secondaries:
            economy_str += " (" + ", ".join(_economy_display(e) for e in secondaries) + ")"
        lines.append(f"Economy: {economy_str}")
    if not has_market:
        return lines
    ...
    if not isinstance(categories, dict):
        return lines
    produced_skus: set[str] = set()
    consumed_skus: set[str] = set()
    for row in categories.values():
        if isinstance(row, dict):
            produced_skus.update(row.get("produced") or [])
            consumed_skus.update(row.get("consumed") or [])
    if produced_skus:
        lines.append("Produces:")
        for name in [_get_good_display_name(engine, sku) for sku in sorted(produced_skus)]:
            lines.append(f"  {name}")
    if consumed_skus:
        lines.append("Consumes:")
        for name in [_get_good_display_name(engine, sku) for sku in sorted(consumed_skus)]:
            lines.append(f"  {name}")
    return lines


def _print_destination_context(engine: GameEngine) -> None:
    """Destination summary: full Destination and System Emoji Profiles; market profile in player-facing form."""
    ...
    dest_name = ctx.get("destination_name", "Unknown")
    dest_entity = _destination_display_entity(dest_obj) if dest_obj else SimpleNamespace(emoji_id=None, tier=None, tags=[])
    dest_display = format_entity_name(dest_entity, dest_name) if dest_visible else dest_name
    system_name = ctx.get("system_name", "Unknown")
    system_display = format_entity_name(system_obj, system_name) if sys_visible else system_name
    print(f"Destination: {dest_display}")
    ...
    if system_government:
        ...
        print()
    has_market = dest_obj is not None and getattr(dest_obj, "market", None) is not None
    market_lines = _format_market_profile_lines(engine, ctx, has_market)
    for line in market_lines:
        print(line)
    system_id_ctx = ctx.get("system_id", "")
    destination_id = engine.player_state.current_destination_id
    situation_rows = engine._active_situation_rows_for_system(system_id=system_id_ctx) if system_id_ctx else []
    system_situations = []
    ...
    for row in situation_rows:
        ...
        if isinstance(situation_id, str) and situation_id:
            if scope == "system":
                system_situations.append(situation_id.replace("_", " ").title())
            elif scope == "destination" and target_id == destination_id:
                destination_situations.append(situation_id.replace("_", " ").title())
...
def _galaxy_summary(engine: GameEngine) -> None:
    ...
    for system in systems:
        ...
        for dest in destinations:
            visible_d = dest.destination_id in _visited_destination_ids(engine)
            dest_display = format_entity_name(_destination_display_entity(dest), dest.display_name) if visible_d else dest.display_name
            print(f"    {dest_display}")
            primary_str = _economy_display(dest.primary_economy_id) if getattr(dest, "primary_economy_id", None) else "—"
            secondary_str = ", ".join(_economy_display(e) for e in (dest.secondary_economy_ids or [])) or "—"
            print(f"      Economy: {primary_str}")
            if dest.secondary_economy_ids:
                print(f"      Also: {secondary_str}")
        print()
...
def _destination_actions_menu(engine: GameEngine) -> None:
    ...
    while True:
        ...
        print("0) Back")
        for index, action in enumerate(actions, start=1):
            display = _entity_display(action, str(action.get("display_name", "") or action.get("action_id", "")), True)
            print(f"{index}) {display}")
...
def _market_buy_menu(engine: GameEngine) -> None:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        leg = row.get("legality", "--")
        risk = row.get("risk_tier", "--")
        print(f"{index}) {display_name} | price={row.get('unit_price')} | legality={leg} | risk={risk}")
    raw_index = input("Select good to buy (number): ").strip()
    raw_qty = input("Quantity: ").strip()
    try:
        selected = int(raw_index)
        quantity = int(raw_qty)
...
def _market_sell_menu(engine: GameEngine) -> None:
    ...
    for index, row in enumerate(rows, start=1):
        sku_id = row.get("sku_id", "")
        display_name = _get_good_display_name(engine, sku_id) if sku_id else row.get("display_name", "Unknown")
        cargo = row.get("player_has_units", 0)
        leg = row.get("legality", "--")
        risk = row.get("risk_tier", "--")
        print(f"{index}) {display_name} | cargo={cargo} | price={row.get('unit_price')} | legality={leg} | risk={risk}")
    raw_index = input("Select good to sell (number): ").strip()
    raw_qty = input("Quantity: ").strip()
    try:
        selected = int(raw_index)
        quantity = int(raw_qty)
...
def _print_market_sku_overlay(engine: GameEngine) -> None:
    ...
    for sku_id in all_skus:
        ...
        if isinstance(legality, str) and legality:
            leg_str = legality
        else:
            leg_str = str(legality) if legality is not None else "--"
        print(f"  {display_name} | buy={buy_price} | sell={sell_price} | cargo={cargo_units} | legality={leg_str} | risk={risk_str}")
...
def _print_datanet_profile(profile: dict[str, object]) -> None:
    ...
    system_situations = [str(s).replace("_", " ").title() for s in profile.get("system_situations") or []]
    dest_situations = [str(s).replace("_", " ").title() for s in profile.get("destination_situations") or []]
    print("  System situations: " + (", ".join(system_situations) if system_situations else "None"))
    print("  Destination situations: " + (", ".join(dest_situations) if dest_situations else "None"))
    ...
    if missions:
        ...
        for index, mission in enumerate(missions, start=1):
            if not isinstance(mission, dict):
                continue
            title = mission.get("title") or mission.get("mission_type", "Mission")
            display = _format_name_with_profile(mission, str(title), True)
            print(f"    {index}) {display}")
    else:
        print("  Available missions: None")
        print("No missions available.")
    notices = [str(n).replace("_", " ").title() for n in profile.get("system_notices") or []]
    print("  System notices: " + (", ".join(notices) if notices else "None"))


def _accept_mission_from_datanet(engine: GameEngine, profile: dict[str, object]) -> None:
    """Accept a mission from DataNet using the same missions just displayed."""
    missions = [m for m in profile.get("available_missions", []) if isinstance(m, dict)]
    if not missions:
        print("No missions available.")
        return

    # Show the same missions again so the player can pick one.
    print("\nDataNet Missions:")
    for index, mission in enumerate(missions, start=1):
        title = mission.get("title") or mission.get("name") or mission.get("mission_type", "Mission")
        display = _format_name_with_profile(mission, str(title), True)
        print(f"  {index}) {display}")

    raw = input("Select mission index to accept [0 cancel]: ").strip()
    if raw == "0":
        return
    try:
        selected = int(raw)
    ...
    if _is_game_over_result(result):
        ...
        return
    if result.get("ok"):
        # Try to derive a nice title; fall back carefully.
        detail = _extract_detail_from_stage(step_result=result, stage="mission")
        if isinstance(detail, dict) and detail.get("accepted"):
            result_mission = detail.get("mission") or {}
            if isinstance(result_mission, dict):
                title = (
                    result_mission.get("name")
                    or result_mission.get("title")
                    or result_mission.get("mission_type")
                )
            else:
                title = None
            if not title:
...
def _print_warehouse_profile(engine: GameEngine, profile: dict[str, object]) -> None:
    ...
    if isinstance(stored_goods, dict) and stored_goods:
        for sku_id in sorted(stored_goods):
            display_name = _get_good_display_name(engine, sku_id)
            print(f"    {display_name}: {stored_goods[sku_id]}")
    else:
        print("    none")
...
def _format_cargo_for_display(engine: GameEngine, manifest: object) -> str:
    """Format cargo dict as player-facing line: 'GoodName qty, ...' or 'none'."""
    compact = _compact_manifest(manifest)
    if not compact:
        return "none"
    parts = []
    for sku_id in sorted(compact.keys()):
        qty = compact[sku_id]
        name = _get_good_display_name(engine, sku_id)
        parts.append(f"{name} {qty}")
    return ", ".join(parts)


def _get_ship_headline(engine: GameEngine, ship_info: Dict[str, Any] | dict[str, Any]) -> str:
    """
    Build ship headline using Emoji Profile schema: [primary] NAME [tier] [secondary]...

    NAME prefers ship.display_name, then hull display name. Falls back to generic label.
    """
    if not isinstance(ship_info, dict):
        return "Unknown Ship"

    # Resolve display name: prefer ship.display_name, then hull display name.
    try:
    ...
    hull_id = ship_info.get("hull_id", "") or ""
    base_name = ship_info.get("display_name")
    if not base_name:
        if isinstance(hull_id, str) and hull_id:
            base_name = _get_hull_name(hull_id)
        else:
            base_name = "Unknown Ship"

    # Resolve Emoji Profile entity from hull definition so tier/frame come from data.
    entity = None
    try:
        ...
        for hull in hulls_data.get("hulls", []):
            if not isinstance(hull, dict):
                continue
            if str(hull.get("hull_id", "")) != str(hull_id):
                continue
            entity = SimpleNamespace(
                emoji_id=hull.get("emoji_id"),
                tier=hull.get("tier"),
    ...
    if entity is None:
        entity = _normalize_entity_for_display(ship_info)

    return _format_name_with_profile(entity, str(base_name), True)


def _build_module_catalog() -> Dict[str, Dict[str, Any]]:
    """Load module definitions keyed by module_id for Emoji Profile enrichment."""
...
def _get_ship_modules_display_lines(engine: GameEngine, ship_info: Dict[str, Any] | dict[str, Any]) -> List[str]:
    """
    Return installed module display lines using Emoji Profile schema where possible.

    Primary path:
    - Use engine.get_ship_modules(ship_id) and resolve each module's display_name/name
      through _format_name_with_profile.

    Fallback path:
    - If only ship_info.installed_modules (IDs) are available, resolve via modules
      catalog when possible to get player-facing names and Emoji Profiles. As a
      last resort, raw IDs are returned.
    """
    lines: List[str] = []
    module_catalog = _build_module_catalog()
    ship_id = ship_info.get("ship_id") if isinstance(ship_info, dict) else None

    # Primary: use engine's rich module records for this ship.
    ...
    if modules:
        for module in modules:
            ...
            # Prefer display_name, then definition name, then module.name, then raw id.
            name = module.get("display_name")
            if not name and module_def:
                name = module_def.get("name")
            if not name:
                name = module.get("name")
            if not name:
                name = module_id or "Unknown Module"
            if module_def:
                # Module profile is tag-driven: builder resolves tags -> tags.json -> emoji.
                tags_from_def = module_def.get("tags") or []
                if not tags_from_def and module_def.get("primary_tag"):
                    tags_from_def = [module_def["primary_tag"]]
                entity = SimpleNamespace(
    ...
    # Fallback: resolve installed_modules IDs via modules catalog where possible.
    installed_ids = []
...
# Player-facing prompts for action parameters (no raw IDs)
_PLAYER_PARAM_PROMPTS = {
    "requested_units": "How many fuel units?",
    "quantity": "Quantity?",
...
def _prompt_action_kwargs(action: dict[str, object]) -> dict[str, object]:
    ...
    for param in params:
        if isinstance(param, dict):
            ...
            param_prompt = _PLAYER_PARAM_PROMPTS.get(param_name) or param.get("prompt") or param_name
            if param_name and param_name in ("ship_id", "hull_id", "module_id", "sku_id"):
                continue
...
def _combat_action_menu(
    ...
    if enemy_ship_info:
        ...
        display_name = format_entity_name(enemy_ship_info, hull_name)
        ...
        print(f"Enemy Ship: {display_name}")
        if module_count > 0:
            module_parts = []
            if weapon_modules > 0:
